...
RUN if [ `dpkg -s postfix >/dev/null 2>&1; echo $?` -eq 0 ]; then apt-get remove -y --purge postfix; fi;
...
RUN apt-get update \
    && apt-get install -y \
    # Apache\PHP
    apache2 libapache2-mod-gnutls php5 php5-mysql php5-curl php-pear php5-gd php-xml-parser php5-dev libpcre3-dev \
    # Mogile
